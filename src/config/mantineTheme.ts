...
export const mantineToTailwindColor = () => {
  const tailwindColors:any = {};

  Object.keys(mantineColor).forEach((color: ExtendedColors) => {
    const mc = mantineColor[color];
    tailwindColors[color] = { DEFAULT : "" };
    mc.forEach((c, i) => {
      tailwindColors[color][i] = c;
    })
    tailwindColors[color].DEFAULT = mc[defaultShades];
  })

  return tailwindColors;
}
/** activate to get setup color for tailwind config */
// console.log(JSON.stringify(mantineToTailwindColor()))
